...
const markdownBoldToHtml = (text: string) => {
  return text.replace(/\*\*(.*?)\*\*/g, (_, boldText) => `<strong>${boldText}</strong>`);
};

// Convert any <h2>/<h3> followed by multiple <p> paragraphs into <ul><li> lists with Tailwind styling
...
const JobCard: FC<JobCardProps> = ({
  ...
  function parseDescription(html?: string) {
    if (!html) return <p className="text-gray-600 italic">No description available.</p>;
    ...
    <div className="min-h-screen bg-gray-50 flex flex-col items-center p-4 sm:p-6 text-black font-inter">
      ...
      <div className="w-full max-w-5xl mt-8 grid grid-cols-1 md:grid-cols-3 gap-6">
        <div className="col-span-2 bg-white rounded-2xl shadow p-8">
          <h2 className="text-2xl font-bold mb-6 border-b border-gray-200 pb-3">Job Description</h2>
        ...
        <div className="space-y-8">
          <div className="bg-white rounded-2xl shadow p-6">
            ...
            <a href="#" className="text-teal-600 hover:underline text-base block mt-4">
              Visit website
            </a>
            <a href="#" className="text-teal-600 hover:underline text-base mt-2 block">
              Careers page
            </a>
          </div>

          <button className="w-full bg-teal-500 text-white py-3 rounded-2xl shadow hover:bg-teal-600 transition text-lg font-semibold">
            Apply Now
          </button>
